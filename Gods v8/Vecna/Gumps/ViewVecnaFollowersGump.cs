...
{
	public class ViewVecnaFollowersGump : Gump
	{
		...
		public ViewVecnaFollowersGump( BookofVecna stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
...
////
                ...
                if (m.A9Faith == A9Faith.FaithInVecna)
                {
					AddLabel(35, offset, 1149, m.Name.ToString() + @" [Follower]" );
		}

                if (m.A9Faith == A9Faith.PriestOfVecna)
                {
					AddLabel(35, offset, 32, m.Name.ToString() + @" [Priest]" );
		}

                if (m.A9Faith == A9Faith.Necromancer)
                {
					AddLabel(35, offset, 0x35, m.Name.ToString() + @" [Necromancer]" );
		}
////
				}
			}
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new ViewVecnaFollowersGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}

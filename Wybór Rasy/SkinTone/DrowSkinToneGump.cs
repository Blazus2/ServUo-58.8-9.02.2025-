...
{
    public class DrowSkinToneGump : Gump
    {
        ///////////////////////////////////////////////////////////
        ...
        private static int[] hues = { 0x76D, 0x384, 0x579, 0x3E9, 0x3E8, 0x3DE, 0x389, 0x385, 0x376, 0x53F, 0x381, 0x382, 0x383, 0x76B, 0x3E5, 0x51D, 0x3E6 }; //Try all the hues you want to use here, can be as little or many as you want, all seperated by a comma. The gump will only show a max of 14 though.
        ...
        {
            CommandSystem.Register("DrowSkinChange", AccessLevel.Counselor, new CommandEventHandler(DrowSkinChange_OnCommand));
        }

        [Usage("")]
        [Description("Command used for testing only..")]
        public static void DrowSkinChange_OnCommand(CommandEventArgs e)
        {
            if (e.Mobile != null)
            {
                if (e.Mobile.HasGump(typeof(DrowSkinToneGump)))
                    e.Mobile.CloseGump(typeof(DrowSkinToneGump));
                e.Mobile.SendGump(new DrowSkinToneGump(e.Mobile));
            }
        }

        public DrowSkinToneGump(Mobile from, int lastselected)
            : base(Start_X, Start_Y)
        {
            PokazDrow_Gump(from, lastselected);
        }

        public DrowSkinToneGump(Mobile from)
            : base(Start_X, Start_Y)
        {
            PokazDrow_Gump(from, Last_Selected);
        }


        private void PokazDrow_Gump(Mobile from, int lastselected)
        {
            try
            {
                ...
                if (caller == null)
                {
                    ConsoleWrite(ConsoleColor.DarkRed, "DrowSkinChangeGump: Error 1.");
                    return;
                }
                this.Closable = true;
            ...
            catch (Exception e) { ConsoleWrite(ConsoleColor.DarkRed, "DrowSkinChangeGump: Error 3: [" + e.Message + "]"); }
        }
        ...
        private void Show_Top_Right_Message()
        {
            ...
            for (int i = 0; i <= (Top_Right_Message.Length / Max_Length); i++)
            {
                ...
                if (tempnum < Top_Right_Message.Length) AddLabel(20, 100 + (Line_Height * i), 42, Top_Right_Message.Substring(tempnum, (tempnum + Max_Length) > Top_Right_Message.Length ? tempnum2 : Max_Length));
                  ...
                  this.AddLabel(190, 20, 567, "KOLORY DROWA");
        }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            try
            {
                ...
                switch (info.ButtonID)
                {
                    case 0:
                        {
                            if (from.HasGump(typeof(DrowSkinToneGump)))
                                from.CloseGump(typeof(DrowSkinToneGump));
                            return;
                        }
                    case 1000:
                        {
                    ...
                    case 1001:
                        {
                            if (Allow_Preview)
                                for (int i = 0; i < hues.Length; i++)
                                    if (info.IsSwitched(i))
                                    {
                                        new PreviewColorDrow(hues[i], from, i);
            ...
            catch (Exception e) { ConsoleWrite(ConsoleColor.DarkRed, "DrowSkinChangeGump: Error 4: [" + e.Message + "]"); }
        }
    ...
    public class PreviewColorDrow
    {
        private int oldhue, PAGE;
        private Mobile from;

        public PreviewColorDrow(int hue, Mobile who, int page)
        {
            from = who;
            oldhue = who.Hue;
            who.Hue = hue;
        ...
        public void BackToNormal()
        {
            ...
            if (from.HasGump(typeof(DrowSkinToneGump)))
                from.CloseGump(typeof(DrowSkinToneGump));
            //from.SendGump(new DrowSkinToneGump(from, PAGE));
        }
    }
}
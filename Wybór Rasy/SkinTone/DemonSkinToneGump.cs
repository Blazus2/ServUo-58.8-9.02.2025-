...
{
    public class DemonSkinToneGump : Gump
    {
        ///////////////////////////////////////////////////////////
        ...
        private static int[] hues = { 37, 47, 57, 67, 77 }; //Try all the hues you want to use here, can be as little or many as you want, all seperated by a comma. The gump will only show a max of 14 though.
        ...
        {
            CommandSystem.Register("DemonSkinChange", AccessLevel.Counselor, new CommandEventHandler(DemonSkinChange_OnCommand));
        }

        [Usage("")]
        [Description("Command used for testing only..")]
        public static void DemonSkinChange_OnCommand(CommandEventArgs e)
        {
            if (e.Mobile != null)
            {
                if (e.Mobile.HasGump(typeof(DemonSkinToneGump)))
                    e.Mobile.CloseGump(typeof(DemonSkinToneGump));
                e.Mobile.SendGump(new DemonSkinToneGump(e.Mobile));
            }
        }

        public DemonSkinToneGump(Mobile from, int lastselected)
            : base(Start_X, Start_Y)
        {
            PokazDemon_Gump(from, lastselected);
        }

        public DemonSkinToneGump(Mobile from)
            : base(Start_X, Start_Y)
        {
            PokazDemon_Gump(from, Last_Selected);
        }


        private void PokazDemon_Gump(Mobile from, int lastselected)
        {
            try
            {
                ...
                if (caller == null)
                {
                    ConsoleWrite(ConsoleColor.DarkRed, "DemonSkinChangeGump: Error 1.");
                    return;
                }
                this.Closable = true;
            ...
            catch (Exception e) { ConsoleWrite(ConsoleColor.DarkRed, "DemonSkinChangeGump: Error 3: [" + e.Message + "]"); }
        }
        ...
        private void Show_Top_Right_Message()
        {
            ...
            for (int i = 0; i <= (Top_Right_Message.Length / Max_Length); i++)
            {
                ...
                if (tempnum < Top_Right_Message.Length) AddLabel(20, 100 + (Line_Height * i), 42, Top_Right_Message.Substring(tempnum, (tempnum + Max_Length) > Top_Right_Message.Length ? tempnum2 : Max_Length));
                  ...
                  this.AddLabel(190, 20, 567, "KOLORY DEMONA");
        }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            try
            {
                ...
                switch (info.ButtonID)
                {
                    case 0:
                        {
                            if (from.HasGump(typeof(DemonSkinToneGump)))
                                from.CloseGump(typeof(DemonSkinToneGump));
                            return;
                        }
                    case 1000:
                        {
                    ...
                    case 1001:
                        {
                            if (Allow_Preview)
                                for (int i = 0; i < hues.Length; i++)
                                    if (info.IsSwitched(i))
                                    {
                                        new PreviewColorDemon(hues[i], from, i);
            ...
            catch (Exception e) { ConsoleWrite(ConsoleColor.DarkRed, "DemonSkinChangeGump: Error 4: [" + e.Message + "]"); }
        }
    ...
    public class PreviewColorDemon
    {
        private int oldhue, PAGE;
        private Mobile from;

        public PreviewColorDemon(int hue, Mobile who, int page)
        {
            from = who;
            oldhue = who.Hue;
            who.Hue = hue;
        ...
        public void BackToNormal()
        {
            ...
            if (from.HasGump(typeof(DemonSkinToneGump)))
                from.CloseGump(typeof(DemonSkinToneGump));
            //from.SendGump(new DemonSkinToneGump(from, PAGE));
        }
    }
}